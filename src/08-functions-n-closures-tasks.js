...
function getPolynom(...arg) {
  const resFunc = (x) => {
    if (arg.length === 3) {
      const resPol = arg[0] * x ** 2 + arg[1] * x + arg[2];
      return resPol;
    }

    if (arg.length === 2) {
      const resPol = arg[0] * x + arg[1];
      return resPol;
    }
...
function memoize(func) {
  ...
  const setStor = (item) => {
    if (storage.has(item) === false) {
      storage.set(item, func(item));
    }
